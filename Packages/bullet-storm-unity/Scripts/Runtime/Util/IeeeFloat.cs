...
namespace CANStudio.BulletStorm.Util
{
    ...
    public struct IeeeFloat : IComparable<float>, IEquatable<float>, IComparable<IeeeFloat>, IEquatable<IeeeFloat>
    {
        ...
        public uint Fraction
        {
            ...
            set
            {
                if (value > MaskFraction) throw new OverflowException($"Value {value} exceeded max value {MaskFraction}");
                bits = value | (bits & ~MaskFraction);
            }
        }

        public IeeeFloat(float value) : this() => Value = value;
        ...
        {
            if (fraction > MaskFraction) throw new OverflowException($"Value {fraction} exceeded max value {MaskFraction}");
        ...
        public bool Equals(IeeeFloat other) => Value.Equals(other.Value);
        public int CompareTo(IeeeFloat other) => Value.CompareTo(other.Value);

        public static implicit operator float(IeeeFloat t) => t.Value;
        public static implicit operator IeeeFloat(float f) => new IeeeFloat(f);

        public override int GetHashCode() => base.GetHashCode();
        public override string ToString() => $"IeeeFloat{{exp: {Exponent}, frac: {Fraction}, value: {Value}}}";
    }
}