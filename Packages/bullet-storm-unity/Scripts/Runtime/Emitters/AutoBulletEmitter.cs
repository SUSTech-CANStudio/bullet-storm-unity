...
namespace CANStudio.BulletStorm.Emitters
{
    ...
    {
        [Header("Bullet config")] [Tooltip("Bullet system prefab to emit bullets.")] [SerializeField] [Required]
        ...
        private void EmitOnce(float emitSpeed, Color emitColor, float emitSize)
        {
            ...
            float SeqTo01(int seq, int cnt)
            {
                return seq == 0 ? 0 : (float) seq / (cnt - 1);
            }
            ...
            foreach (var param in paramList) Emit(param, bullet);
        }
