...
namespace CANStudio.BulletStorm.Emitters
{
    ...
    public abstract class AutoEmitterBase : Emitter
    {
        ...
        [BoxGroup("Auto aim")] [Label("Enable")] [Tooltip("Auto rotates the emitter to aim at a target.")]
        ...
        {
            // init auto aim
            if (enableAutoAim && autoAim.aimOnEmissionStart && autoAim.target.Check()) transform.LookAt(autoAim.target);
        ...
        public override void Emit(StormInfo storm)
        {
            base.Emit(storm, Emitter);
        }

        public override void Emit(IEnumerable<BulletEmitParam> shape, IBullet bullet)
        {
            base.Emit(shape, bullet, Emitter);
        }

        public override void Emit(BulletEmitParam emitParam, IBullet bullet)
        {
            base.Emit(emitParam, bullet, Emitter);
        }
        ...
        public struct AutoAimModule
        {
            ...
            [Header("Follow rate")] [Tooltip("Use a curve to describe follow rate.")]
        ...
        public struct AimOffsetModule
        {
            ...
            [Tooltip("Offset on x-axis.")] public ParticleSystem.MinMaxCurve xOffset;

            [Header("Y-axis")]
            [Tooltip("Offset mode on y-axis, this will cause emitter aim left (negative) and right (positive).")]
            public OffsetMode yOffsetMode;

            [Tooltip("Offset on y-axis.")] public ParticleSystem.MinMaxCurve yOffset;

            [Header("Z-axis")]
            [Tooltip(
                "Offset mode on z-axis, this will cause emitter aim clockwise (negative) and counterclockwise (positive).")]
            public OffsetMode zOffsetMode;

            [Tooltip("Offset on z-axis.")] public ParticleSystem.MinMaxCurve zOffset;
