...
namespace CANStudio.BulletStorm.BulletSystem
{
	...
	/// , so you can simply attach it onto a
	/// <see cref="GameObject" />
	/// ,
	/// and modify attributes in inspector.
	/// <para />
	/// It works with a Unity
	/// <see cref="ParticleSystem" />
	/// component, any settings on the component or on this script can work together.
    ...
    public class ParticleBullet : BulletBase
    {
        [Tooltip("Enable custom speed over lifetime, this will override emit speed.")]
        [Label("Enable")]
        [SerializeField]
        [BoxGroup("Speed over lifetime")]
        private bool enableSpeedOverLifetime;

        [Label("Detail")] [SerializeField] [BoxGroup("Speed over lifetime")] [EnableIf("enableSpeedOverLifetime")]
        ...
        public override void Emit(BulletEmitParam relative, Transform emitter)
        {
            ParticleSystem.EmitParams ToEmitParams(in BulletEmitParam bulletEmitParam)
            {
                ...
                if (!bulletEmitParam.DefaultColor) result.startColor = bulletEmitParam.color;
                if (!bulletEmitParam.DefaultSize) result.startSize3D = bulletEmitParam.size;
        ...
        private void UpdateParticles()
        {
            ...
            if (particles is null || particles.Length < psm.maxParticles)
        ...
        private struct SpeedOverLifetimeModule
        {
            ...
            {
                return speed.Evaluate(normalizedLifetime) * speedMultiplier;
            }
        }
    }
}